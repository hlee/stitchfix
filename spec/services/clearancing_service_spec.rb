...
describe ClearancingService do
  ...
  describe "::process_file" do
    ...
    context "total success" do
      ...
      before do
        #@clearancing_status = ClearancingService.new("barcodes", "123&23,45").process_arry
        cs = ClearancingService.new("barcodes", item_ids)
        cs.process_arry
        bs = BaseService.new(cs)
        bs.clearance_items!
    ...
    context "partial success" do
      ...
      let(:uploaded_file)     { Rack::Test::UploadedFile.new(file_name) }
      let(:u_file)            { double(:file) }
      before do
        allow(u_file).to receive(:tempfile).and_return(uploaded_file)
        #@clearancing_status = clearancing_service.process_file(uploaded_file)
        cs = ClearancingService.new("file", u_file)
        cs.process_arry
        bs = BaseService.new(cs)
        bs.clearance_items!
        @clearancing_status = bs.clearancing_status
